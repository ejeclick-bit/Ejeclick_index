...
*   [x] **Story 1.1b**: Configurar Husky + lint-staged + commitlint (pre-commit hooks).
...
*   [x] **Story 3.4b**: Conectar formulario a backend real (FastAPI endpoint `/api/v1/leads`).
...
*   [x] **Story 4.2**: Configurar Vitest + React Testing Library y escribir tests unitarios (8 tests).
...
*   [x] **Story 4.3b**: Code splitting: separar vendor chunks (Three.js, framer-motion) con lazy loading.
*   [x] **Story 4.4**: Inserción de schemas JSON-LD (Organization, LocalBusiness).
*   [x] **Story 4.4b**: Añadir schema FAQPage para el acordeón de preguntas frecuentes.
...
*   [x] **Story 5.1**: Configurar Dockerfile + docker-compose (frontend Vite + backend FastAPI + PostgreSQL).
*   [x] **Story 5.2**: Construir backend FastAPI con endpoint `/api/v1/leads` y conexión PostgreSQL.
...
*   [x] **Story 5.5**: Configurar CSP headers y seguridad de producción (nginx.conf).
