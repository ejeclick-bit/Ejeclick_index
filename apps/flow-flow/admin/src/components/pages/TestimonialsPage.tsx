  ...
  return (
    <div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onClick={() => setShowForm(false)}>
          <div className="w-full max-w-md rounded-xl border border-neutral-800 bg-brand-card p-6" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-semibold text-white">{editing ? 'Editar' : 'Nuevo'} Testimonio</h3>
              <button onClick={() => setShowForm(false)} className="text-neutral-500 hover:text-white"><X size={20} /></button>
            </div>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-sm text-neutral-300 mb-1">Cita</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-sm text-neutral-300 mb-1">Autor</label>
      ...
      <div className="space-y-3">
        {items.length === 0 ? (
          <p className="text-sm text-neutral-500">No hay testimonios. Crea el primero.</p>
        ) : (
          items.map((t) => (
            <div key={t.id} className="rounded-xl border border-neutral-800 bg-brand-card p-4">
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1">
                  <p className="text-sm text-neutral-300 italic">"{t.quote}"</p>
                  <p className="mt-2 text-sm font-medium text-white">{t.author}</p>
                  {t.role && <p className="text-xs text-neutral-500">{t.role}</p>}
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  <button onClick={() => openEdit(t)} className="text-neutral-500 hover:text-white"><Pencil size={16} /></button>
                  <button onClick={() => handleDelete(t.id)} className="text-neutral-500 hover:text-red-400"><Trash2 size={16} /></button>
                </div>
              </div>
            </div>
          ))
