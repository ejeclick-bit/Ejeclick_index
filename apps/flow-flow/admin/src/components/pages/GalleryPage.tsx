        ...
        <>
          ...
          {images.length === 0 ? (
            ...
            <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
              {images.map((img) => (
                <div key={img.id} className="group relative rounded-xl border border-subtle bg-brand-card overflow-hidden">
                  ...
                  <button
                    ...
                    className="absolute top-2 right-2 rounded-lg bg-red-500/90 px-3 py-1.5 text-xs text-foreground opacity-0 group-hover:opacity-100 transition-opacity hover:bg-red-500 disabled:opacity-50 flex items-center gap-1"
                  >
                    {deleting === img.id ? <Loader2 size={12} className="animate-spin" /> : <Trash2 size={12} />}
                    Eliminar
                  </button>
                  <span className="absolute top-2 left-2 rounded-full bg-black/60 px-2 py-0.5 text-[10px] text-foreground/80">
